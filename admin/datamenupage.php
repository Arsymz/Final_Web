                    ...
                    <div class="card border-0">
                        <div class="card-header">
                            <div class="d-flex justify-content-between">
                                <h2 class="fw-bold text-white">DATA menu</h2>
                                <a href="tambahmenupage.php" class="btn btn-secondary fw-bold">Tambah menu</a>
                            </div>
                        </div>
                        <div class="card-body">
...
$keyword=$_POST['keyword'];
    if ($keyword !=''){
        $query = mysqli_query($koneksi, "SELECT * FROM menu INNER JOIN kategori ON menu.id_kategori=kategori.id_kategori WHERE nama_menu LIKE '%$keyword%'");
        
    }else{
        $query = mysqli_query($koneksi, "SELECT * FROM menu INNER JOIN kategori ON menu.id_kategori=kategori.id_kategori ORDER BY id_menu ASC LIMIT $start_from, $limit ");
    }
    while ($data=mysqli_fetch_array($query)) {
?>
                                    <tr>
                                        <th scope="row"><?= $no++ ?></th>
                                        <td style="width:150px; font-size:15px;"><b><?= $data['nama_menu'] ?><b></td>
                                        <td><?= $data['nama_kategori'] ?></td>
                                        <td><?= $data['nama_cafe']?></td>
                                        <td><?=$data['tgl_menu']?></td>
                                        <td>
                                            <img src="../images/<?= $data['gambar_menu'] ?>" class="img-fluid" style="width:100px; height:100px;">
                                        </td>
                                        <td>
                                            <div class="btn-group" style="margin-top:30px;">
                                                <a href="editmenupage.php?id_menu=<?= $data['id_menu'] ?>" class="btn btn-dark" style="width:50px; font-size:12px; margin:2px;">Edit</a>
                                                <a href="deletemenu.php?id_menu=<?= $data['id_menu'] ?>" class="btn btn-secondary delete-link" style="width:70px; font-size:12px; margin:2px;">Delete</a>
                                            </div>
                                        </td>
                                    </tr>
                                    <?php
                            ...
                            $pagLink = "<ul class='pagination'>";  
                                for ($i=1; $i<=$total_pages; $i++) {
                                    $pagLink .= "<li class='page-item'><a class='page-link bg-dark text-white' href='datamenupage.php?page=".$i."'>".$i."</a></li>";	
                                }
                            echo $pagLink . "</ul>";  
                            ?>
                        </div>
